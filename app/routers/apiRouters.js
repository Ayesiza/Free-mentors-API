...
router.get('/mentors', auth.getToken,auth.verifyUserToken,  user.getAllMentors);
router.get('/mentor/:id', auth.getToken, auth.verifyUserToken,auth.checkParamsInPut,auth.getUserById, user.specificMentor);
router.post('/sessions',auth.getToken, auth.verifyUserToken ,review.sessionAlreadyExist,review.checkIfMentor,review.ifMentorExists, session.createSession);
